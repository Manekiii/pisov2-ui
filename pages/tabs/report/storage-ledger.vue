...
const onInit = async (ipage) => {
  ...
  const response = await serviceApi().get(
    "pallet/get-storage-ledger-running-balance/?brancode=" +
      sitecode +
      "&loccode=" +
      scope.appLocation +
      "&dtefrom=" +
...
const onPreview = function () {
  ...
  if (
    ...
    dteto == undefined ||
    scope.appLocation == "" ||
    scope.appLocation == undefined
  ) {
    scope.requiredPrompt = "Please fill up required fields.";
  } else {
    scope.locationDes = filterFilter(scope.location, {
      Location: scope.appLocation,
    })[0].LocationDescription;
    scope.onInit(1);
